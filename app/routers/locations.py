...
async def list_canteens(
    college_id: Optional[UUID] = Query(None, alias="collegeId"),
    ...
    if college_id:
        query = query.join(Canteen.colleges).where(College.id == college_id)
    result = await db.execute(query)
...
async def suggest_college(request: SuggestCollegeRequest, db: AsyncSession = Depends(get_db)):
    """
